            ...
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-700">
                    {routes.map((route) => (
                        ...
                        >
                            <td className="px-6 py-4 font-bold text-white max-w-[320px]">
                                <div className="flex flex-wrap gap-1.5 max-h-[80px] overflow-y-auto">
                                    {route.host ? (
                                        route.host.split(',').map((h) => {
                                            const trimmed = h.trim();
                                            return trimmed ? (
                                                <span key={trimmed} className="px-2 py-0.5 text-xs bg-gray-800/80 text-blue-200 border border-blue-900/50 rounded-md font-mono break-all inline-block">
                                                    {trimmed}
                                                </span>
                                            ) : null;
                                        })
                                    ) : (
                                        <span className="text-gray-500 italic text-xs">Any host</span>
                                    )}
                                </div>
                            </td>
