            ...
            <header className="flex justify-between items-start">
                <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                        {route.host ? (
                            route.host.split(',').map((h) => {
                                const trimmed = h.trim();
                                return trimmed ? (
                                    <span key={trimmed} className="px-2.5 py-1 text-sm bg-gray-800/85 text-blue-200 border border-blue-900/50 rounded-md font-mono font-semibold">
                                        {trimmed}
                                    </span>
                                ) : null;
                            })
                    ...
                    <h1 className="text-xl font-bold text-gray-400 font-mono">
                        Path: <span className="text-white bg-gray-900/80 px-2 py-0.5 rounded border border-gray-800">{route.path || "/"}</span>
                    </h1>
                    <p className="text-gray-400 text-sm flex items-center flex-wrap gap-y-1">
                        <span>Target:</span>
                        <span className="text-blue-400 font-mono ml-1">{route.target_service}:{route.target_port}</span>
                        <span className="mx-2">•</span>
                        <span>Deployment:</span>
                        <span className="text-blue-400 font-mono ml-1 mr-2">{route.deployment}</span>
                        {route.always_on && (
                            <span className="px-1.5 py-0.5 text-[10px] bg-green-900/60 text-green-200 border border-green-800/60 rounded font-semibold select-none">
                                Always On
                            </span>
                        )}
                    </p>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-mono text-gray-300">
                            {formatDuration(route.idle_timeout)}
                        </div>
